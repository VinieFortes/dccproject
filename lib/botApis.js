...
const { fetchJson, uploadImages} = require('../utils/fetcher')
...
async function imgNeko(mentionedJidList ,bot, user, sender, id, neko, from, isMedia, type, isQuotedImage, quotedMsg, message, uaOverride, author) {
    if (mentionedJidList.length !== 0) {
        const userPic = await bot.getProfilePicFromServer(user)
        const dataPpPh = await bent ('buffer') (userPic)
        const pic = await uploadImages (dataPpPh, `foto.${sender.id}`)
        await bot.reply (from, "Calma ai parceiro que to rodando numa calculadora", id)
        const lixo = await axios.get (`https://nekobot.xyz/api/imagegen?type=${neko}&url=${pic}`)
        ...
        const fotolix = await uploadImages(datapotogay, `foto.${sender.id}`)
        await bot.reply(from, "Calma ai parceiro que to rodando numa calculadora", id)
        const lixo = await axios.get(`https://nekobot.xyz/api/imagegen?type=${neko}&url=${fotolix}`)
        const tumh = lixo.data
        let ext;
        if (tumh.message.endsWith('.png')) {
            ext = '.png';
        ...
        await bot.reply(from, "Calma ai parceiro que to rodando numa calculadora", id)
        const lixo2 = await axios.get(`https://nekobot.xyz/api/imagegen?type=${neko}&url=${fotolix}`)
